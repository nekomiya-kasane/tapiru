...
TEST(VirtualListTest, RendersVisibleItems) {
    virtual_terminal vt;
    auto con = vt.make_console(false, 40);
    auto vl = virtual_list_builder(100, 5)
        .item_builder([](uint32_t idx) {
            return text_builder("Row " + std::to_string(idx));
        });
...
TEST(VirtualListTest, ScrollOffsetShiftsWindow) {
    ...
    auto vl = virtual_list_builder(100, 3)
        .item_builder([](uint32_t idx) {
            return text_builder("R" + std::to_string(idx));
        })
        .scroll_offset(10);
    con.print_widget(std::move(vl), 40);
    auto& out = vt.raw();
...
TEST(VirtualListTest, ScrollOffsetClampedToMax) {
    ...
    auto vl = virtual_list_builder(10, 5)
        .item_builder([](uint32_t) { return text_builder("x"); })
        .scroll_offset(999);
    con.print_widget(std::move(vl), 40);
    auto& out = vt.raw();
