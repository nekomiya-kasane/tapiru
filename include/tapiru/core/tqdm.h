...
public:
    ...
    void init(size_t total, const std::string& desc, int bar_width,
              const std::string& unit, bool disable, bool leave,
        ...
        *p++ = '\r';
        ...
        if (total_ > 0) {
            ...
            if (colour_.r || colour_.g || colour_.b) {
                n = std::snprintf(p, static_cast<size_t>(end - p),
                    "\x1b[38;2;%u;%u;%um", colour_.r, colour_.g, colour_.b);
                if (n > 0) p += n;
            }

            for (int i = 0; i < filled && p < end; ++i) {
                // UTF-8 for █ (U+2588): 0xE2 0x96 0x88
                if (p + 3 <= end) { *p++ = '\xe2'; *p++ = '\x96'; *p++ = '\x88'; }
            }
            ...
            *p++ = ' ';
            ...
            if (current > 0 && current < total_) {
                double remaining = elapsed / static_cast<double>(current)
                                   * static_cast<double>(total_ - current);
            ...
            n = std::snprintf(p, static_cast<size_t>(end - p),
                " [%s<%s, %s]", elapsed_buf, eta_buf, rate_buf);
            if (n > 0) p += n;
        } else {
            ...
            int n = std::snprintf(p, static_cast<size_t>(end - p),
                "%zu%s [%s, %s]", current, unit_.c_str(), elapsed_buf, rate_buf);
            if (n > 0) p += n;
        }
...
public:
    explicit tqdm_range(Range& range)
        : range_(&range) {
        total_ = compute_size(range);
    }

    tqdm_range(Range& range, const char* description)
        : range_(&range), desc_(description) {
        total_ = compute_size(range);
    }

    // Builder methods (return *this for chaining)
    tqdm_range& desc(const char* d)       { desc_ = d; return *this; }
    tqdm_range& desc(const std::string& d){ desc_ = d; return *this; }
    tqdm_range& bar_width(int w)          { bar_width_ = w; return *this; }
    tqdm_range& unit(const char* u)       { unit_ = u; return *this; }
    tqdm_range& unit(const std::string& u){ unit_ = u; return *this; }
    tqdm_range& total(size_t t)           { total_ = t; return *this; }
    tqdm_range& disable(bool d = true)    { disable_ = d; return *this; }
    tqdm_range& leave(bool l = true)      { leave_ = l; return *this; }
    tqdm_range& miniters(int m)           { miniters_ = m; return *this; }
    tqdm_range& mininterval(double s)     { mininterval_ = s; return *this; }
    tqdm_range& colour(color c)           { colour_ = c; return *this; }
    tqdm_range& file(FILE* f)             { file_ = f; return *this; }
    ...
    public:
        ...
        iterator(Iter it, detail::tqdm_bar* bar, size_t idx)
            : it_(it), bar_(bar), idx_(idx) {}
    ...
    iterator begin() {
        bar_.init(total_, desc_, bar_width_, unit_, disable_, leave_,
                  miniters_, mininterval_, colour_, file_);
        bar_.update(0);
        return iterator(std::begin(*range_), &bar_, 0);
    }

    iterator end() {
        return iterator(std::end(*range_), nullptr, total_);
    }
...
private:
    template <typename R>
    static size_t compute_size(R& r) {
...
class trange_range {
public:
    trange_range(T start, T stop, T step)
        : range_(start, stop, step), total_(range_.size()) {}

    trange_range& desc(const char* d)       { desc_ = d; return *this; }
    trange_range& desc(const std::string& d){ desc_ = d; return *this; }
    trange_range& bar_width(int w)          { bar_width_ = w; return *this; }
    trange_range& unit(const char* u)       { unit_ = u; return *this; }
    trange_range& unit(const std::string& u){ unit_ = u; return *this; }
    trange_range& total(size_t t)           { total_ = t; return *this; }
    trange_range& disable(bool d = true)    { disable_ = d; return *this; }
    trange_range& leave(bool l = true)      { leave_ = l; return *this; }
    trange_range& miniters(int m)           { miniters_ = m; return *this; }
    trange_range& mininterval(double s)     { mininterval_ = s; return *this; }
    trange_range& colour(color c)           { colour_ = c; return *this; }
    trange_range& file(FILE* f)             { file_ = f; return *this; }
    ...
    public:
        ...
        iterator(inner_iter it, detail::tqdm_bar* bar, size_t idx)
            : it_(it), bar_(bar), idx_(idx) {}
    ...
    iterator end() {
        return iterator(range_.end(), nullptr, total_);
    }
