...
public:
  ...
  template <typename... Args>
  void print_inline(std::format_string<Args...> fmt, Args &&...args) {
    print_inline(
        std::string_view{std::format(fmt, std::forward<Args>(args)...)});
  }
  ...
  void print_widget(const Builder &builder, uint32_t width = 0);

  /// @brief Render with bounded width AND height (enables flex layout).
  template <typename Builder>
  void print_widget(const Builder &builder, uint32_t width, uint32_t height);
  ...
  template <typename... Args>
  void print_highlighted(std::format_string<Args...> fmt, Args &&...args) {
    print_highlighted(
        std::string_view{std::format(fmt, std::forward<Args>(args)...)});
  }
  ...
  template <typename... Args>
  void print_hl(std::format_string<Args...> fmt, Args &&...args) {
    print_highlighted(
        std::string_view{std::format(fmt, std::forward<Args>(args)...)});
  }
  ...
  template <typename... Args>
  void print_highlighted_inline(std::format_string<Args...> fmt,
                                Args &&...args) {
    print_highlighted_inline(
        std::string_view{std::format(fmt, std::forward<Args>(args)...)});
  }

  /** @brief Alias for print_highlighted_inline. */
  void print_hl_inline(std::string_view text) {
    print_highlighted_inline(text);
  }

  /** @brief Alias for print_highlighted_inline with std::format-style
   * arguments. */
  template <typename... Args>
  void print_hl_inline(std::format_string<Args...> fmt, Args &&...args) {
    print_highlighted_inline(
        std::string_view{std::format(fmt, std::forward<Args>(args)...)});
  }
  ...
  [[nodiscard]] const highlighter *get_highlighter() const noexcept {
    return highlighter_;
  }
  ...
  void emit_rich_plan(const static_rich_markup<MaxFragments> &plan,
                      const char *src);
  ...
  template <typename... Args>
  void print_rich(std::format_string<Args...> fmt, Args &&...args) {
...
private:
  ...
  void render_widget_impl(const std::function<uint32_t(void *)> &flatten_fn,
                          void *builder_ptr, uint32_t width);
  void render_widget_impl(const std::function<uint32_t(void *)> &flatten_fn,
                          void *builder_ptr, uint32_t width, uint32_t height);
...
struct TAPIRU_API widget_canvas {
    ...
    [[nodiscard]] const cell& cell_at(uint32_t x, uint32_t y) const {
        return cv.get(x, y);
    }

    /** @brief Get the style of the cell at (x, y). */
    [[nodiscard]] style style_at(uint32_t x, uint32_t y) const {
        return styles.lookup(cv.get(x, y).sid);
    }
 ...
 */
TAPIRU_API widget_canvas render_to_canvas_impl(
    const std::function<uint32_t(void*)>& flatten_fn, uint32_t width);

template <typename Builder>
widget_canvas render_to_canvas(const Builder& builder, uint32_t width) {
...
template <typename Builder>
void console::print_widget(const Builder &builder, uint32_t width) {
...
template <typename Builder>
void console::print_widget(const Builder &builder, uint32_t width, uint32_t height) {
  render_widget_impl(
      [&builder](void *scene_ptr) -> uint32_t {
        auto &s = *static_cast<detail::scene *>(scene_ptr);
        return builder.flatten_into(s);
...
template <size_t MaxFragments>
void console::emit_rich_plan(const static_rich_markup<MaxFragments> &plan,
                             const char *src) {
  std::string buf;
  buf.reserve(256);
