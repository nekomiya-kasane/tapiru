...
public:
    ...
    template <typename B>
    accordion_builder& add_section(std::string_view header, B&& content) {
        sections_.push_back({
            std::string(header),
    ...
    accordion_builder& expanded(const std::vector<bool>* exp)  { expanded_ = exp; return *this; }
    accordion_builder& header_style(const style& s)            { header_sty_ = s; return *this; }
    accordion_builder& active_header_style(const style& s)     { active_header_sty_ = s; return *this; }
    accordion_builder& border(border_style bs)                 { border_ = bs; return *this; }
    accordion_builder& z_order(int16_t z)                      { z_order_ = z; return *this; }
