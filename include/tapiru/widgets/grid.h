...
public:
    ...
    template <typename B>
    grid_builder& add(B&& child) {
        cells_.push_back(std::make_unique<model<std::decay_t<B>>>(std::forward<B>(child)));
        return *this;
    }

    grid_builder& columns(uint32_t c)                   { columns_ = c; return *this; }
    grid_builder& row_gap(uint32_t g)                    { row_gap_ = g; return *this; }
    grid_builder& col_gap(uint32_t g)                    { col_gap_ = g; return *this; }
    grid_builder& z_order(int16_t z)                     { z_order_ = z; return *this; }
