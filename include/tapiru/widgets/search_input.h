...
namespace tapiru {
    ...
    class TAPIRU_API search_input_builder {
      public:
        ...
        search_input_builder &query(const std::string *q) {
            query_ = q;
            return *this;
        }
        search_input_builder &match_count(const int *c) {
            match_count_ = c;
            return *this;
        }
        search_input_builder &current_match(const int *c) {
            current_match_ = c;
            return *this;
        }
        search_input_builder &placeholder(std::string_view p) {
            placeholder_ = p;
            return *this;
        }
        search_input_builder &width(uint32_t w) {
            width_ = w;
            return *this;
        }
        search_input_builder &input_style(const style &s) {
            input_sty_ = s;
            return *this;
        }
        search_input_builder &match_style(const style &s) {
            match_sty_ = s;
            return *this;
        }
        search_input_builder &border(border_style bs) {
            border_ = bs;
            return *this;
        }
        search_input_builder &z_order(int16_t z) {
            z_order_ = z;
            return *this;
        }
