...
struct spinner_frames {
    ...
    static const std::vector<std::string>& arc() {
        static const std::vector<std::string> f = {
            "\xe2\x97\x9c", "\xe2\x97\xa0", "\xe2\x97\x9d", "\xe2\x97\x9f"
        };  // ◜◠◝◟
        return f;
    }
};
...
public:
    explicit spinner_builder(std::shared_ptr<spinner_state> state)
        ...
        , frames_(spinner_frames::dots()) {}

    spinner_builder& message(std::string_view msg) { message_ = msg; return *this; }
    spinner_builder& frames(std::vector<std::string> f) { frames_ = std::move(f); return *this; }
    spinner_builder& spinner_style(const style& s) { spinner_sty_ = s; return *this; }
    spinner_builder& done_text(std::string_view t) { done_text_ = t; return *this; }
    spinner_builder& key(std::string_view k);
    spinner_builder& z_order(int16_t z) { z_order_ = z; return *this; }
