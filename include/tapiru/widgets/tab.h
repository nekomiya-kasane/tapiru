...
public:
    ...
    template <typename B>
    tab_builder& add_tab(std::string_view label, B&& content) {
        tabs_.push_back({
            std::string(label),
            std::make_unique<model<std::decay_t<B>>>(std::forward<B>(content))
        });
        return *this;
    }

    tab_builder& active(const int* idx)              { active_ = idx; return *this; }
    tab_builder& tab_style(const style& s)           { tab_sty_ = s; return *this; }
    tab_builder& active_tab_style(const style& s)    { active_sty_ = s; return *this; }
    tab_builder& content_border(border_style bs)     { content_border_ = bs; return *this; }
    tab_builder& content_border_style(const style& s){ content_border_sty_ = s; return *this; }
    tab_builder& z_order(int16_t z)                  { z_order_ = z; return *this; }
