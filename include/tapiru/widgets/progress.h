...
class TAPIRU_API progress_task {
  public:
    progress_task(std::string description, uint64_t total)
        : description_(std::move(description)), total_(total), current_(0), completed_(false) {}

    /** @brief Advance progress by delta. Thread-safe. */
    void advance(uint64_t delta) noexcept { current_.fetch_add(delta, std::memory_order_relaxed); }

    /** @brief Set absolute progress value. Thread-safe. */
    void set_current(uint64_t value) noexcept { current_.store(value, std::memory_order_relaxed); }
    ...
    void set_total(uint64_t total) noexcept { total_.store(total, std::memory_order_relaxed); }
...
class TAPIRU_API progress_builder {
  public:
    ...
    progress_builder &bar_width(uint32_t w) {
        bar_width_ = w;
        return *this;
    }
    progress_builder &bar_style(const style &s) {
        bar_sty_ = s;
        return *this;
    }
    progress_builder &complete_char(char32_t ch) {
        complete_ch_ = ch;
        return *this;
    }
    progress_builder &remaining_char(char32_t ch) {
        remaining_ch_ = ch;
        return *this;
    }
    progress_builder &key(std::string_view k);
    progress_builder &z_order(int16_t z) {
        z_order_ = z;
        return *this;
    }
