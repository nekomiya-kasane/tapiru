...
public:
    ...
    void activate(int index)   { active_ = index; open_ = true; }
    void deactivate()          { open_ = false; }
    void move_left(int count)  { if (count > 0) active_ = (active_ - 1 + count) % count; }
    void move_right(int count) { if (count > 0) active_ = (active_ + 1) % count; }

    void set_selected(int global_index) { selected_ = global_index; was_selected_ = true; }
    void clear_selection() { selected_ = -1; was_selected_ = false; }
...
public:
    ...
    menu_bar_builder& state(const menu_bar_state* s)        { state_ = s; return *this; }
    menu_bar_builder& bar_style(const style& s)             { bar_sty_ = s; return *this; }
    menu_bar_builder& active_style(const style& s)          { active_sty_ = s; return *this; }
    menu_bar_builder& highlight_style(const style& s)       { highlight_sty_ = s; return *this; }
    menu_bar_builder& shortcut_style(const style& s)        { shortcut_sty_ = s; return *this; }
    menu_bar_builder& item_style(const style& s)            { item_sty_ = s; return *this; }
    menu_bar_builder& dropdown_border(border_style bs)      { dropdown_border_ = bs; return *this; }
    menu_bar_builder& z_order(int16_t z)                    { z_order_ = z; return *this; }
