...
class TAPIRU_API virtual_list_builder {
  public:
    ...
    virtual_list_builder &item_builder(item_fn fn) {
        item_fn_ = std::move(fn);
        return *this;
    }
    virtual_list_builder &scroll_offset(uint32_t offset) {
        offset_ = offset;
        return *this;
    }
    virtual_list_builder &key(std::string_view k) {
        key_ = detail::fnv1a_hash(k);
        return *this;
    }
    virtual_list_builder &z_order(int16_t z) {
        z_order_ = z;
        return *this;
    }
