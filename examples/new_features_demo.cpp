...
static void demo_status_bar(console &con) {
    ...
    con.print_widget(status_bar_builder().left("tapiru v0.1.0").right("Ready"), W);
}
...
static void demo_popup(console &con) {
    ...
    {
        auto content = text_builder("[bold]Are you sure you want to exit?[/]\n"
                                    "\n"
    ...
    {
        auto content = text_builder("[bold cyan]Notification[/]\n"
                                    "\n"
...
static void demo_widget_keys(console &con) {
    ...
    {
        rows_builder rb;
        rb.add(text_builder("[bold]text_builder[/] with .key(\"header\")").key("header"));
        rb.add(text_builder("[bold]panel_builder[/] with .key(\"info-panel\")").key("info-panel-text"));
        rb.gap(0);
...
static void demo_complex_layout(console &con) {
    ...
    subsection(con, "8a. Dashboard: header + 2-column body + status bar:");
    {
        ...
        // Header
        dashboard.add(text_builder("[bold bright_white on_blue]  Dashboard — tapiru v0.1.0  [/]").key("dash-header"));
        ...
        // Body: two columns
        {
            ...
            // Left column: progress tasks
            {
                ...
                panel_builder left_panel(
                    progress_builder().add_task(t1).add_task(t2).add_task(t3).bar_width(18).key("sys-progress"));
        ...
        // Footer: status bar
        dashboard.add(status_bar_builder()
                          .left("[bold] ONLINE [/]")
...
static void demo_interactive_keys(console &con) {
    ...
    con.print_widget(button_builder("Submit")
                         .key("submit-btn")
    ...
    {
        bool checked = true;
        con.print_widget(checkbox_builder("Enable notifications", &checked).key("notif-cb"), 35);
    }
    ...
    {
        float val = 0.7f;
        con.print_widget(slider_builder(&val, 0.0f, 1.0f).width(30).show_percentage(true).key("volume-slider"), 45);
    }
    ...
    subsection(con, "9d. Text input with key:");
    {
        ...
        con.print_widget(text_input_builder(&buf)
                             .width(30)
...
static void demo_live_dashboard(console &con) {
...

    {
        ...
        for (int frame = 0; frame <= 30; ++frame) {
            ...
            dashboard.add(text_builder("[bold bright_white on_blue]  Live System Monitor  [/]"));

            {
                columns_builder body;
                body.add(progress_builder().add_task(cpu_task).add_task(mem_task).add_task(disk_task).bar_width(25).key(
                             "live-progress"),
                         1);
                body.gap(1);
                body.key("live-body");
...
int main() {
    ...
    con.newline();
    con.print_widget(rule_builder(" tapiru — New Features Comprehensive Demo ")
    ...
    con.newline();
    con.print_widget(rule_builder(" New Features Demo Complete! ")
