...
static void show_header(console& con) {
    con.newline();
    con.print_widget(
        rule_builder(" gin — Feature & Performance Dashboard ")
...
static void show_performance(console& con) {
    ...
    struct perf_entry { const char* name; double us; int ops; };
    perf_entry entries[] = {
        {"ustring create (short)",      0.15,  1000000},
        {"ustring::lines() iterate",    2.5,   100000},
        {"flat_string_map insert",      0.08,  1000000},
        {"flat_string_map lookup",      0.05,  1000000},
        {"Base64 encode (1KB)",         1.8,   500000},
        {"Base64 decode (1KB)",         2.1,   500000},
        {"JSON parse (small obj)",      3.5,   200000},
        {"JSON dump (small obj)",       1.2,   500000},
        {"Thread pool submit",          0.4,   1000000},
        {"Logger info() call",          0.6,   1000000},
        {"Config get_int()",            0.03,  5000000},
        {"Signal emit (3 slots)",       0.25,  2000000},
    };
    ...
    con.print_widget(
        bar_chart_builder(throughputs, 6)
            .labels(labels)
            .style_override(style{colors::bright_yellow}),
        W);
}
...
int main() {
    ...
    con.newline();
    con.print_widget(
        rule_builder(" gin Dashboard Complete ")
