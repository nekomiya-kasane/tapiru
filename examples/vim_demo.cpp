...
static void do_undo(editor_state& st) {
    if (st.undo_stack.empty()) { st.status_msg = "Already at oldest change"; st.status_msg_ticks = 40; return; }
...
static void do_redo(editor_state& st) {
    if (st.redo_stack.empty()) { st.status_msg = "Already at newest change"; st.status_msg_ticks = 40; return; }
    st.undo_stack.push_back({st.lines, st.cursor_row, st.cursor_col});
...
static const char* mode_string(vim_mode m) {
    switch (m) {
    case vim_mode::normal:      return "-- NORMAL --";
    case vim_mode::insert:      return "-- INSERT --";
    case vim_mode::visual:      return "-- VISUAL --";
    case vim_mode::visual_line: return "-- VISUAL LINE --";
    case vim_mode::command:     return "";
    case vim_mode::search:      return "";
    }
    return "";
}
...
static text_range compute_selection(const editor_state& st) {
    if (st.mode == vim_mode::visual) {
        return {st.visual_anchor_row, st.visual_anchor_col,
                st.cursor_row, st.cursor_col + 1};
    }
...
static void yank_selection(editor_state& st) {
    ...
    if (st.yank_linewise) {
        ...
        if (sr == er) {
            st.yank_buf.push_back(st.lines[static_cast<size_t>(sr)].substr(
...
static bool execute_command(editor_state& st, classic_app& app, const std::string& cmd) {
    ...
    for (char c : cmd) { if (!std::isdigit(static_cast<unsigned char>(c))) { all_digits = false; break; } }
    if (all_digits) {
...
static bool handle_normal(editor_state& st, classic_app& app, const key_event& ke) {
    ...
    if (st.g_pending) {
        st.g_pending = false;
        if (ke.codepoint == 'g') { st.cursor_row = 0; st.cursor_col = 0; clamp_cursor(st); return true; }
        return true;
    }
    if (st.d_pending) {
        st.d_pending = false;
    ...
    if (ke.codepoint == '0') { st.cursor_col = 0; return true; }
    if (ke.codepoint == '$') { st.cursor_col = line_len > 0 ? line_len - 1 : 0; return true; }
    if (ke.codepoint == 'G') { st.cursor_row = total - 1; st.cursor_col = 0; clamp_cursor(st); return true; }
    if (ke.codepoint == 'g') { st.g_pending = true; return true; }
    if (ke.codepoint == 'H') { st.cursor_row = app.scroll_y(); clamp_cursor(st); return true; }
    if (ke.codepoint == 'M') { st.cursor_row = app.scroll_y() + app.viewport_h() / 2; clamp_cursor(st); return true; }
    if (ke.codepoint == 'L') { st.cursor_row = app.scroll_y() + app.viewport_h() - 1; clamp_cursor(st); return true; }
    ...
    // Mode switches
    if (ke.codepoint == 'i') { st.mode = vim_mode::insert; return true; }
    if (ke.codepoint == 'a') {
        st.mode = vim_mode::insert;
        if (line_len > 0) ++st.cursor_col;
        return true;
    }
    if (ke.codepoint == 'I') { st.mode = vim_mode::insert; st.cursor_col = 0; return true; }
    if (ke.codepoint == 'A') { st.mode = vim_mode::insert; st.cursor_col = line_len; return true; }
    if (ke.codepoint == 'o') {
        push_undo(st);
    ...
    if (ke.codepoint == ':') { st.mode = vim_mode::command; st.command_buf.clear(); return true; }
    if (ke.codepoint == '/') { st.mode = vim_mode::search; st.command_buf.clear(); return true; }
    ...
    if (ke.codepoint == 'd') { st.d_pending = true; return true; }
    if (ke.codepoint == 'y') { st.y_pending = true; return true; }
    if (ke.codepoint == 'p') {
    ...
    if (ke.codepoint == 'u') { do_undo(st); return true; }
    if (ke.key == special_key::none && ke.codepoint == 18) { // Ctrl+R
        do_redo(st); return true;
    }
...
static bool handle_insert(editor_state& st, const key_event& ke) {
    ...
    // Arrow keys
    if (ke.key == special_key::up)    { if (st.cursor_row > 0) --st.cursor_row; clamp_cursor(st); return true; }
    if (ke.key == special_key::down)  { if (st.cursor_row < static_cast<int>(st.lines.size()) - 1) ++st.cursor_row; clamp_cursor(st); return true; }
    if (ke.key == special_key::left)  { if (st.cursor_col > 0) --st.cursor_col; return true; }
    if (ke.key == special_key::right) { ++st.cursor_col; clamp_cursor(st); return true; }
...
static bool handle_visual(editor_state& st, const key_event& ke) {
    ...
    if (ke.codepoint == 'h' || ke.key == special_key::left) { if (st.cursor_col > 0) --st.cursor_col; return true; }
    if (ke.codepoint == 'l' || ke.key == special_key::right) { if (st.cursor_col < line_len - 1) ++st.cursor_col; return true; }
    if (ke.codepoint == 'j' || ke.key == special_key::down) { if (st.cursor_row < total - 1) ++st.cursor_row; clamp_cursor(st); return true; }
    if (ke.codepoint == 'k' || ke.key == special_key::up) { if (st.cursor_row > 0) --st.cursor_row; clamp_cursor(st); return true; }
    if (ke.codepoint == 'w') { st.cursor_col = find_word_forward(st.lines[static_cast<size_t>(st.cursor_row)], st.cursor_col); clamp_cursor(st); return true; }
    if (ke.codepoint == 'b') { st.cursor_col = find_word_backward(st.lines[static_cast<size_t>(st.cursor_row)], st.cursor_col); return true; }
    if (ke.codepoint == '0') { st.cursor_col = 0; return true; }
    if (ke.codepoint == '$') { st.cursor_col = line_len > 0 ? line_len - 1 : 0; return true; }
    if (ke.codepoint == 'G') { st.cursor_row = total - 1; clamp_cursor(st); return true; }

    // Switch visual modes
    if (ke.codepoint == 'v' && st.mode == vim_mode::visual_line) { st.mode = vim_mode::visual; return true; }
    if (ke.codepoint == 'V' && st.mode == vim_mode::visual) { st.mode = vim_mode::visual_line; return true; }
    ...
    if (ke.key == special_key::enter) {
        ...
        if (!st.search_matches.empty()) {
            ...
            for (int i = 0; i < static_cast<int>(st.search_matches.size()); ++i) {
                ...
                if (m.start_row > st.cursor_row ||
                    (m.start_row == st.cursor_row && m.start_col >= st.cursor_col)) {
                    best = i;
                    break;
                }
            }
            ...
            char buf[64];
            std::snprintf(buf, sizeof(buf), "/%s  [%d/%d]", st.search_pattern.c_str(),
                          st.current_match + 1, static_cast<int>(st.search_matches.size()));
            st.status_msg = buf;
            st.status_msg_ticks = 40;
        } else {
...
static void handle_menu_action(editor_state& st, classic_app& app, const std::string& label) {
    if (label == "Quit") { app.quit(); return; }
    if (label == "Save & Quit") { app.quit(); return; }
    if (label == "Save") {
        st.status_msg = "\"" + st.filename + "\" written (simulated)";
        st.status_msg_ticks = 40;
        return;
    ...
    if (label == "Undo") { do_undo(st); return; }
    if (label == "Redo") { do_redo(st); return; }
    ...
    if (label == "Go to Top") { st.cursor_row = 0; st.cursor_col = 0; return; }
    if (label == "Go to Bottom") { st.cursor_row = static_cast<int>(st.lines.size()) - 1; st.cursor_col = 0; clamp_cursor(st); return; }
...
int main() {
    ...
    app.set_status([&](status_bar_builder& sb) {
        ...
        char buf[64];
        std::snprintf(buf, sizeof(buf), "Ln %d, Col %d  UTF-8",
                      st.cursor_row + 1, st.cursor_col + 1);
        sb.right(buf);
    });

    app.on_menu_action([&](int, int, const std::string& label) {
        handle_menu_action(st, app, label);
    });
    ...
    app.on_key([&](const key_event& ke) -> bool {
        ...
        if (st.popup_open) {
            if (ke.key == special_key::escape || ke.codepoint == 'q' ||
                ke.key == special_key::enter) {
                st.popup_open = false;
