...
static void demo_scroll_view(console& con) {
    ...
    subsection(con, "Scrollable viewport (5 visible lines, scrolled to line 3):");
    {
        ...
        for (int i = 0; i < 20; ++i) {
            content.add(text_builder("  Line " + std::to_string(i + 1) +
...
static void demo_tab(console& con) {
    ...
    subsection(con, "Tab view with 3 tabs (second tab active):");
    {
        ...
        tabs.add_tab("General", text_builder(
            "  [bold]Application Name:[/] tapiru\n"
            "  [bold]Version:[/]          0.2.0\n"
            "  [bold]License:[/]          MIT"
        ));
        tabs.add_tab("Editor", text_builder(
            "  [bold]Font:[/]       Cascadia Code\n"
            "  [bold]Size:[/]       14px\n"
            "  [bold]Theme:[/]      Dark Modern\n"
            "  [bold]Tab Size:[/]   4 spaces"
        ));
        tabs.add_tab("Keybindings", text_builder(
            "  [bold]Ctrl+S[/]  Save\n"
            "  [bold]Ctrl+Z[/]  Undo\n"
...
static void demo_transitions(console& con) {
    ...
    {
        typewriter tw("The quick brown fox jumps over the lazy dog.",
                      duration_ms(2000));
    ...
    subsection(con, "6b. Counter animation (counting from 0 to 1000, at 75%):");
    {
        ...
        con.print_widget(text_builder("[bold bright_yellow]Downloads: [/][bold]" +
                                      std::to_string(val) + "[/]"), W);
    }
    ...
    {
        marquee mq("  Breaking News: tapiru v0.2 released with 17 new widgets!  ",
                    40, duration_ms(150));
...
static void demo_accordion(console& con) {
    ...
    subsection(con, "Collapsible sections (first and third expanded):");
    {
        ...
        acc.add_section("Getting Started", text_builder(
            "    Install tapiru via CMake:\n"
            "    [cyan]add_subdirectory(tapiru)[/]\n"
            "    [cyan]target_link_libraries(app PRIVATE tapiru)[/]"
        ));
        acc.add_section("API Reference", text_builder(
            "    (collapsed — click to expand)"
        ));
        acc.add_section("Examples", text_builder(
            "    See the [bold]examples/[/] directory for demos:\n"
...
static void demo_tree_view(console& con) {
    ...
    subsection(con, "File tree with expanded directories:");
    {
        ...
        std::unordered_set<std::string> expanded = {
            "tapiru/", "include/", "tapiru/", "core/", "src/"
        };
        int cursor = 3;
...
static void demo_toggle(console& con) {
    ...
    subsection(con, "Toggle switches with different states:");
    {
        ...
        rb.add(toggle_builder()
            .value(&off).label("Auto Save")
            .on_style(style{colors::bright_green, {}, attr::bold})
            .off_style(style{colors::bright_black})
            .label_style(style{colors::white})
        );
        rb.add(toggle_builder()
            .value(&on).label("Line Numbers")
...
static void demo_toast(console& con) {
    ...
    subsection(con, "Toast messages at different levels:");
    {
        ...
        rb.add(toast_builder()
            .state(&ts_info)
            .info_style(style{colors::bright_cyan})
        ...
        rb.add(toast_builder()
            .state(&ts_warn)
            .warning_style(style{colors::bright_yellow})
            .border(border_style::rounded)
        );
        ...
        rb.add(toast_builder()
            .state(&ts_err)
            .error_style(style{colors::bright_red})
            .border(border_style::rounded)
        );
...
static void demo_grid(console& con) {
    ...
    subsection(con, "2x3 grid of status cards:");
    {
        auto card = [](const char* title, const char* value, const style& sty) {
            auto panel = panel_builder(text_builder(
                std::string("[bold]") + title + "[/]\n" + value
            ));
            panel.border(border_style::rounded);
    ...
    subsection(con, "Combining multiple new widgets into a dashboard:");
    {
        ...
        // Breadcrumb navigation
        {
            ...
            dash.add(breadcrumb_builder()
                .add_item("Dashboard").add_item("Monitoring").add_item("Overview")
                .active(&active)
...
int main() {
    ...
    con.newline();
    con.print_widget(
        rule_builder(" tapiru \xe2\x80\x94 Widget Expansion Demo ")
    ...
    con.newline();
    con.print_widget(
        rule_builder(" Widget Expansion Demo Complete! ")
