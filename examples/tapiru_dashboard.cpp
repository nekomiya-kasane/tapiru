...
template <typename F>
static bench_result bench(const char* name, int ops, F&& fn) {
...
static void show_header(console& con) {
    con.newline();
    con.print_widget(
        rule_builder(" tapiru — Feature & Performance Dashboard ")
...
static void show_perf_benchmarks(console& con) {
    ...
    results.push_back(bench("Markup parse (long)", 50000, [] {
        volatile auto spans = parse_markup(
            "[bold]Title[/] [dim]subtitle[/] [italic cyan on_#112233]styled[/] "
...
int main() {
    ...
    con.newline();
    con.print_widget(
        rule_builder(" tapiru Dashboard Complete ")
