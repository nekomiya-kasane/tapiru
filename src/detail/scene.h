...
#define TAPIRU_DEFINE_POOL_ACCESSORS(name, Data)                                                                       \
    ...
    [[nodiscard]] Data &get_##name(pool_index i) {                                                                     \
        return name##s_[i];                                                                                            \
    }                                                                                                                  \
    [[nodiscard]] const Data &get_##name(pool_index i) const {                                                         \
        return name##s_[i];                                                                                            \
    }

#define TAPIRU_DEFINE_POOL_MEMBER(name, Data) std::vector<Data> name##s_;
...
#define TAPIRU_DEFINE_POOL_CLEAR(name) name##s_.clear();
    ...
    class scene {
      public:
        ...
        node_id add_node(widget_type type, pool_index pidx, node_id parent = no_node, widget_key key = no_key) {
