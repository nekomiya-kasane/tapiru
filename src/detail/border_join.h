...
inline border_style classify_border_char(char32_t cp) {
    ...
    switch (cp) {
        case U'\x256D': case U'\x256E': case U'\x2570': case U'\x256F':
        case U'\x2500': case U'\x2502': case U'\x253C':
        case U'\x252C': case U'\x2534': case U'\x251C': case U'\x2524':
            return border_style::rounded;
        case U'\x250F': case U'\x2513': case U'\x2517': case U'\x251B':
        case U'\x2501': case U'\x2503': case U'\x254B':
        case U'\x2533': case U'\x253B': case U'\x2523': case U'\x252B':
            return border_style::heavy;
        case U'\x2554': case U'\x2557': case U'\x255A': case U'\x255D':
        case U'\x2550': case U'\x2551': case U'\x256C':
        case U'\x2566': case U'\x2569': case U'\x2560': case U'\x2563':
...
inline bool has_horizontal(char32_t cp, border_style bs) {
    ...
    return cp == bc.horizontal || cp == bc.top_left || cp == bc.top_right ||
           cp == bc.bottom_left || cp == bc.bottom_right || cp == bc.cross ||
           cp == bc.t_down || cp == bc.t_up || cp == bc.t_right || cp == bc.t_left;
}
...
inline bool has_vertical(char32_t cp, border_style bs) {
    auto bc = get_border_chars(bs);
    return cp == bc.vertical || cp == bc.top_left || cp == bc.top_right ||
           cp == bc.bottom_left || cp == bc.bottom_right || cp == bc.cross ||
           cp == bc.t_down || cp == bc.t_up || cp == bc.t_right || cp == bc.t_left;
}
...
inline char32_t select_join(bool up, bool down, bool left, bool right, border_style bs) {
    ...
    switch (mask) {
        case 15: return bc.cross;        // ┼  all four
        case 14: return bc.t_down;       // ┬  down+left+right
        case 13: return bc.t_up;         // ┴  up+left+right
        case 11: return bc.t_right;      // ├  up+down+right
        case  7: return bc.t_left;       // ┤  up+down+left
        case 10: return bc.top_left;     // ╭  down+right
        case  6: return bc.top_right;    // ╮  down+left
        case  9: return bc.bottom_left;  // ╰  up+right
        case  5: return bc.bottom_right; // ╯  up+left
        case 12: return bc.horizontal;   // ─  left+right
        case  3: return bc.vertical;     // │  up+down
        default: return 0;
    }
}
