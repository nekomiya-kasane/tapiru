...
std::vector<md_block> parse_markdown(std::string_view md) {
    ...
    while (i < md.size()) {
        ...
        if (line.size() >= 3) {
            ...
            if (rc == '-' || rc == '*' || rc == '_') {
                for (size_t j = 0; j < line.size(); ++j) {
                    if (line[j] != rc && line[j] != ' ') { is_rule = false; break; }
                }
                if (is_rule) {
        ...
        if (line[0] == '#') {
            ...
            blocks.push_back({md_block_type::heading, level,
        ...
        if (line.size() >= 2 && line[0] == '>' && line[1] == ' ') {
            blocks.push_back({md_block_type::blockquote, 0,
                              std::string(line.substr(2))});
            continue;
        }
        if (line.size() >= 1 && line[0] == '>') {
            blocks.push_back({md_block_type::blockquote, 0,
                              std::string(line.substr(1))});
            continue;
        }
        ...
        if (line.size() >= 5 && line[0] == '-' && line[1] == ' ' &&
            line[2] == '[' && (line[3] == 'x' || line[3] == 'X' || line[3] == ' ') && line[4] == ']') {
            md_block blk;
        ...
        if (line.size() >= 2 && (line[0] == '-' || line[0] == '*') && line[1] == ' ') {
            blocks.push_back({md_block_type::list_item, 0,
                              std::string(line.substr(2))});
            continue;
        }
...
node_id markdown_builder::flatten_into(detail::scene& s) const {
    ...
    for (size_t bi = 0; bi < blocks_.size(); ++bi) {
        ...
        case md_block_type::heading: {
            ...
            if (blk.level == 1) markup += " bright_white on_blue";
            else if (blk.level == 2) markup += " bright_cyan";
            else markup += " bright_yellow";
            markup += "]";
...
std::string syntax_highlight(std::string_view code, std::string_view language) {
    ...
    static const std::unordered_set<std::string> cpp_keywords = {
        ...
        "typename", "using", "virtual", "void", "volatile", "while",
        "int", "float", "double", "char", "bool", "long", "short",
        "unsigned", "signed", "include", "define", "ifdef", "ifndef",
        "endif", "pragma", "override", "final", "concept", "requires",
