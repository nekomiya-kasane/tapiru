...
void console::render_widget_impl(
    const std::function<uint32_t(void *)> &flatten_fn, void * /*builder_ptr*/,
    uint32_t width) {
...
void console::render_widget_impl(
  ...
  auto bc = (height > 0)
      ? box_constraints::tight(width, height)
      : box_constraints::loose(width, unbounded);
...
widget_canvas render_to_canvas_impl(
